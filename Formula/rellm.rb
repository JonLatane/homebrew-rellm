...
# (create_homebrew_release job) in https://github.com/jonlatane/rellm on every
# release to main. Don't hand-edit it -- changes will be overwritten. The launcher
# script below is sourced from docs/rellm_homebrew.sh in that repo.
class Rellm < Formula
  desc "Rellm federated social server"
  homepage "https://github.com/jonlatane/rellm"
  url "https://github.com/jonlatane/rellm/releases/download/v0.5.553-20260923183508-34c3f36/rellm-0.5.553-20260923183508-34c3f36-macos-arm64.tar.gz"
  sha256 "9ae2667e9365ee9651b8c25066042d05de9fb2717af746cfc87d838d783c9996"
  version "0.5.553-20260923183508-34c3f36"
  ...
  def install
    (etc/"rellm").install Dir["*"]
    (bin/"rellm").write <<~SCRIPT
                #!/bin/bash
                #
                ...
                Commands:
                  ...
                  Deployment (requires `make` -- manage your own Kubernetes cluster):
                    ...
                    deploy <targets...>      Run `make` targets from the bundled deploys/Makefile, e.g.:
                                             ...
                                             default. See #{etc}/rellm/opt/deploys/README.md (bundled
                ...
                # (rellm-server, delete_expired_tokens, grpcurl, ...) from #{etc}/rellm.
                _rellm_exec_bin() {
                  local bin="$1"
                  shift
                ...
                jobs() {
                  cd "#{etc}/rellm" && exec ./background_jobs.sh "$@"
                }
                ...
                _rellm_deploys_dir="#{etc}/rellm/opt/deploys"
    ...
    SCRIPT
    chmod 0755, bin/"rellm"

    # bin/"rellm" implements `completion bash`/`completion zsh` (see
    # docs/rellm_homebrew.sh) as a positional arg, not a `--shell=`
    # flag, so shell_parameter_format is left at its nil default --
    # Homebrew's :arg format actually means `--shell=bash`, despite
    # the name. This runs `rellm completion bash`/`completion zsh`
